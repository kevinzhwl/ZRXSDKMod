...
public:
    ...
    ZcadToolImpl()
    {
        wcscpy(m_szLayer,L"0");
        wcscpy(m_FlyoutTools,L"\0");
        dispColor=dispLayer=0;
        m_bColorUnspecified=m_bLayerUnspecified=TRUE;
        m_hWndEditWindow=NULL;
        m_pToolProp=NULL;
        mcfZWCAD = (CLIPFORMAT)::RegisterClipboardFormat(CF_AUTOCAD);
    }
    ~ZcadToolImpl()
    {
...
public:
    ...
    virtual HRESULT GetPropertyTypeInfo()
    {
        ...
        for(int i = 0; i < numFuncs; i++)
        {
            ...
            if(memberID < 1000)
            {
                    ...
                    CComBSTR bstrVal(pBstrNames[0]);
                    COLE2T szName(bstrVal);
                    TOOL_MAP_ENTRY mapEntry;
                    wcscpy(mapEntry.szDesc,szName);
    ...
    STDMETHOD(SetFlyoutTools) ( VARIANT* pFlyoutToolIds)
    {
        ...
        for(long i=lLower; i<=lUpper; i++) {
            ...
            wcscpy(m_FlyoutTools,CW2T(val.bstrVal));
        }
        return S_OK;
    }
