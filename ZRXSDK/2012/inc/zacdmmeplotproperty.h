...
public:
    ...
    void SetNamespace(const wchar_t* ns)
    {
        delete m_namespace;
        if (NULL != ns) {
            m_namespace = new wchar_t[::wcslen(ns) + 1];
            ::wcscpy(m_namespace, ns);
        } else {
            m_namespace = NULL;
        }
    }
    ...
    void SetNamespaceUrl(const wchar_t* nsUrl)
    {
        delete m_namespaceUrl;
        if (NULL != nsUrl) {
            m_namespaceUrl = new wchar_t[::wcslen(nsUrl) + 1];
            ::wcscpy(m_namespaceUrl, nsUrl);
        } else {
            m_namespaceUrl = NULL;
        }
    }
    ...
    void SetAttributeName(const wchar_t* attName)
    {
        delete m_attributeName;
        if (NULL != attName) {
            m_attributeName = new wchar_t[::wcslen(attName) + 1];
            ::wcscpy(m_attributeName, attName);
        } else {
    ...
    void SetAttributeValue(const wchar_t* attValue)
    {
        delete m_attributeValue;
        if (NULL != attValue) {
            m_attributeValue = new wchar_t[::wcslen(attValue) + 1];
            ::wcscpy(m_attributeValue, attValue);
        } else {
...
typedef std::vector<ZcDMMEPlotProperty> ZcDMMEPlotPropertyVec;
