...
ZcDbSymbolTablePointer<T_OBJECT>::ZcDbSymbolTablePointer(
    ...
    : ZcDbObjectPointerBase<T_OBJECT>()
{
    m_status = (pDb == NULL) ? Zcad::eNullObjectPointer
...
{
    if (name == NULL)
        m_status = Zcad::eInvalidInput;
    else {
        ZcDbSymbolTablePointer<T_OBJECT::TableType>
            pTable(pDb, ZcDb::kForRead);
        m_status = pTable.openStatus();
        if (m_status == Zcad::eOk)
...
{
    ZcDbObjectPointerBase::operator =(pObject);
}
