...
typedef std::vector<ZcDMMWideString> ZcDMMStringVec;
...
public:
    ...
    ZcDMMNode(int number, wchar_t * name)
    {
        m_nodeNumber = number;
        if (NULL != name) {
            m_nodeName = new wchar_t[::wcslen(name) + 1];
            ::wcscpy(m_nodeName, name);
        } else
            m_nodeName = NULL;
    }
    ...
    int nodeNumber() {return m_nodeNumber;};
    void SetNodeNumber(int number) {m_nodeNumber = number;};
    const wchar_t * nodeName() {return m_nodeName;};
...
public:
    ...
    {
        if (NULL != role) {
            m_role = new wchar_t[::wcslen(role) + 1];
            ::wcscpy(m_role, role);
        } else
            m_role = NULL;

        if (NULL != mime) {
            m_mime = new wchar_t[::wcslen(mime) + 1];
            ::wcscpy(m_mime, mime);
        } else
            m_mime = NULL;
    ...
    const wchar_t * GetRole() {return m_role;};
    ...
    const wchar_t * GetMime() {return m_mime;};
    ...
    const wchar_t * GetPath() {return m_path;};
...
typedef std::vector<ZcDMMResourceInfo> ZcDMMResourceVec;
