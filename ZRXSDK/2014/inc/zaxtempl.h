...
#ifdef _WIN64
    STDMETHOD(get_ObjectID32)(ZSoft::Int32 *pVal)
    {
        if(pVal)
            *pVal = ZcAxGetObjectId32(m_objRef.objectId().asOldId());
        return S_OK;
    }

    STDMETHOD(get_OwnerID32)(ZSoft::Int32 *pVal)
    {
        ZSoft::IntDbId objId;
        HRESULT hr = ZcAxGetOwnerId(m_objRef, &objId);
        if(pVal)
            *pVal = ZcAxGetObjectId32(objId);
        return hr;
    }
#endif
